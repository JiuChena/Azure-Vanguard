...
namespace Unity.Pipeline.Editor.Commands.Materials
{
    ...
    static class MaterialCommands
    {
            ...
            [CliArg("material", "Reference to the .mat asset (or a loaded material) to read (path / guid / globalId / instanceId).", Required = true)] ObjectRef material)
        {
            ...
            // 2022 compat: Material.rawRenderQueue is Unity 6+ only. The serialized m_CustomRenderQueue
            // holds -1 when the material inherits from the shader and a positive integer when explicitly
            // overridden — the same raw value, preserving the round-trip contract with
            // set_material_properties renderQueue:-1 (inherit).
            var matSo = new SerializedObject(mat);
            var customRQ = matSo.FindProperty("m_CustomRenderQueue");
            int rawRenderQueue = customRQ != null ? customRQ.intValue : mat.renderQueue;
